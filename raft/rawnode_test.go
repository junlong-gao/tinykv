...
func TestRawNodeRestart2AC(t *testing.T) {
	...
	if !reflect.DeepEqual(rd, want) {
		if rd.SoftState != want.SoftState {
			panic("here")
		}
		if rd.HardState.Term != want.HardState.Term {
			panic("here")
		}
		if rd.HardState.Vote != want.HardState.Vote {
			panic("here")
		}
		if rd.HardState.Commit != want.HardState.Commit {
			panic("here")
		}
		if !reflect.DeepEqual(rd.Entries, want.Entries) {
			if len(rd.Entries) != len(want.Entries) {
				panic("here")
			}
			t.Log(len(rd.Entries))
			t.Log(len(want.Entries))
			for i := 0; i < len(rd.Entries); i++ {
				if !reflect.DeepEqual(rd.Entries[i], want.Entries[i]) {
					panic("here")
				}
			}
			panic("here")
